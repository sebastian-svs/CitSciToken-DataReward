...
def test_A2():
    ...
    # randomize first reward account regrister so the contract can request payments and deposits
    ...
    time.sleep(get_random_number(60))

    # Sellers claim CitSciTokens from orderbook
    for i in contract_sensor_seller:
        try:
            contract_sensortrade1.withdrawEarningSeller(
                {"from": i, "gas_limit": 1000000}
            )
            print(i, "withdrew CitSciToken")
        except:
            print(i, "No CitSciToken to withdraw")
        amount = sc_token_contract.balanceOf(i)
        print(i, "has", amount, "CitSciTokens to spend")

    # buy rewards
    for i in contract_sensor_seller:
        reward_selection = random.randint(0, reward_counter - 1)
        print(reward_selection)
    ...
    time.sleep(get_random_number(250))
    ...
    for i in contract_sensor_buyers:
        try:
            ...
            print("order removed of", i)
        except:
            print("already fullfilled of", i)
    ...
    for i in contract_sensor_buyers:
        if contract_sensortrade1.current_price() != 0:
            ...
            print("The current price for this data is:", current_price)
        random_price = get_random_number(current_price)
        random_amount = random.randint(1, 60)
        tx = contract_sensortrade1.addBuyOrder(random_amount, random_price, {"from": i})
        try:
            print(
                "trade happened and data sent",
                tx.events["sendData"],
    ...
    for i in contract_sensor_buyers:
        ...
        random_amount = random.randint(1, 60)
        try:
            tx = contract_sensortrade1.addBuyOrder(
                random_amount, random_price, {"from": i}
            )
        except:
            print("error deleting the rewards previously")
        try:
            print(
                "trade happened and data sent",
                tx.events["sendData"],
